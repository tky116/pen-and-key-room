...
/// 明度スライダー
/// </summary>
public class BrightnessSlider : BaseColorSlider
{
    /// <summary>
    /// テクスチャ生成
    /// </summary>
    protected override void GenerateTexture()
    {
        ...
        var newSprite = ColorPickerTextureGenerator.GenerateBrightnessSlider(size);
        if (newSprite != null && newSprite.texture != null)
        {
            newSprite.texture.name = "BrightnessSliderTexture_" + GetInstanceID();
        }
    ...
    {
        colorPicker.UpdatePreviewFromBrightnessSlider(normalizedValue);
    }
    ...
    {
        colorPicker.UpdateFromBrightnessSlider(normalizedValue);
    }
}
